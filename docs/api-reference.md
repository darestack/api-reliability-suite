...
| `/ready` | `GET` | Public | ✅ Dependency Checks | Dependency-aware readiness for the database and Redis-backed features. |
...
| `/debug/summarize-errors` | `GET` | **Bearer (Admin)** | ✅ Rate Limit | Reads the configured log file, returns a local error summary (count and latest error line), and records which admin requested it. |
