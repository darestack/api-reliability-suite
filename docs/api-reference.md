...
| `/external-api` | `GET` | Public | ✅ **Circuit Breaker** | Proxy to external service. When open, returns a static degraded fallback payload. |
| `/debug/summarize-errors` | `GET` | **Bearer** | ✅ Rate Limit | Reads the configured log file, returns an AI summary, and reports the runtime-selected provider. |
